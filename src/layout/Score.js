...
const AccepXemDiem = (i, index) => {
     const a = cc.filter((c)=> c.name == i.council)
     setC(a)
    const getDatass = async () => {
        const data =  await axios.get(
                  `http://localhost:8080/councilPosition/${a[0].id}`);
   ...
   setTimeout(() => {
    ...
    const getData = async (ID, pss, GV) => {
        if(ID) {
           ...
           const cpID = allccp.filter((a)=> a.userId === ID.userId && a.councilId === c[0].id)
              const ArrayScore = data.data.score.filter((d)=> d.thesisId === i.id && d.councilPositionId === cpID[0].id)
              const cpIDCheck = allccp.filter((a)=> a.councilId === c[0].id)
    ...
    const getDatas = async () => {
        const data = await axios.get(
          ...
          const cpIDCheck = allccp.filter((a)=> a.userId === userNOW.id && a.councilId === c[0].id)
        const ArrayScoreCheck = data.data.score.filter((d)=> d.thesisId === i.id && d.councilPositionId === cpIDCheck[0].id)
                ...
                <table className="theses_table">
                    ...
                    <tbody>
                        {newUI && newUI.map((u, index)=> (
                            ...
                            <th>{u.file && (<button  >
                                    {" "}
                                    <a href={`https://drive.google.com/file/d/${u.file}`} className="dangnopbai show" target="_blank">Xem bài</a>
                            ...
                            {!u.file  && (<button  >
                                    {" "}
                                    <span className="dangnopbai">Chưa có bài nộp</span>
                                    <i className="nopbai">
                            ...
                            <th className="seenscore">{u.totalScore !== 0 && (<><span>{u.totalScore}</span> <i onClick={()=>
                                AccepXemDiem(u, index)
                            }><FontAwesomeIcon icon={faEye}/></i></>)} {u.totalScore === 0 && ("Chưa có")}</th>
                            <th>
                                <button>
                                    {" "}
